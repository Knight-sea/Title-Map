...
import{countCellsWithChildren,isValidParent,findDisplayTerritory}from'../territory/hierarchy.js';
...
function renderTE(tid,c,s){
  ...
  // 子孫含むマス数
  const cells=countCellsWithChildren(tid,s);
  const ch=getColorHex(t.color.hue,t.color.shade);
...
<div style="font-size:10px;color:var(--text-muted);margin-bottom:6px">マス数（子含む）: ${cells}</div>
...
function renderPE(pid,c,s){
  ...
  if(pTerrs.length>0){
    ...
    for(const t of pTerrs){ownedHtml+=`<div class="player-terr-item"><span class="color-dot" style="background:${getColorHex(t.color.hue,t.color.shade)}"></span><span>${esc(t.name||'(名称なし)')}</span><button class="btn-terr-remove" data-tid="${t.id}" title="解除">✕</button></div>`;}
    ownedHtml+='</div></div>';}
