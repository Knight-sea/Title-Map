...
import{getDisplayColor,findBorderInfo,findDisplayTerritory,getSubtreeIds}from'../territory/hierarchy.js';
...
if(ui.selectedTerritoryId){for(const id of getSubtreeIds(ui.selectedTerritoryId,state))hlTids.add(id);}
if(ui.selectedPlayerId){for(const t of getPlayerTerritories(ui.selectedPlayerId)){for(const id of getSubtreeIds(t.id,state))hlTids.add(id);}}
...
// Labels（階層で見える領地のみ・動的フォント）
if(ui.showLabels)this._drawLB(ctx,state,range,scale,ui.viewLevel);
}
...
for(let y=range.y0;y<=range.y1;y++)for(let x=range.x0;x<=range.x1;x++){
const tid=s.cells[y][x].territoryId;if(!tid||!hlTids.has(tid))continue;
const sx=(x-cam.x)*sc,sy=(y-cam.y)*sc;
for(const[dx,dy,x1,y1,x2,y2]of this._dirs(sx,sy,sc)){
const nx=x+dx,ny=y+dy;
const nt=(nx>=0&&nx<s.mapWidth&&ny>=0&&ny<s.mapHeight)?s.cells[ny][nx].territoryId:null;
if(nt!==null&&hlTids.has(nt))continue;// 強調範囲内の内部境界は描かない
ctx.strokeStyle='rgba(60,170,255,0.95)';ctx.lineWidth=3;
...
// ラベル：階層で「見える」領地のみ、動的フォントサイズ
_drawLB(ctx,s,range,sc,vl){
const cam=this.camera;
const bounds=new Map();
for(let y=range.y0;y<=range.y1;y++)for(let x=range.x0;x<=range.x1;x++){
const tid=s.cells[y][x].territoryId;if(!tid)continue;
const dt=findDisplayTerritory(tid,vl);if(!dt)continue;
if(!bounds.has(dt.id))bounds.set(dt.id,{minX:x,minY:y,maxX:x,maxY:y,count:0,t:dt});
const b=bounds.get(dt.id);
if(x<b.minX)b.minX=x;if(x>b.maxX)b.maxX=x;if(y<b.minY)b.minY=y;if(y>b.maxY)b.maxY=y;b.count++;}

ctx.textAlign='center';ctx.textBaseline='middle';
for(const[,b]of bounds){
const t=b.t;if(!t||!t.name)continue;
// スクリーン上の面積からフォントサイズを計算（最小10、最大22）
const fs=Math.max(10,Math.min(22,Math.sqrt(b.count*sc*sc)*0.12));
ctx.font=`bold ${Math.round(fs)}px sans-serif`;
const cx=((b.minX+b.maxX+1)/2-cam.x)*sc;
const cy=((b.minY+b.maxY+1)/2-cam.y)*sc;
const availW=(b.maxX-b.minX+1)*sc*0.85;
const tw=ctx.measureText(t.name).width;
let label=t.name;
if(tw>availW){
let s2=t.name;
while(s2.length>1&&ctx.measureText(s2+'…').width>availW)s2=s2.slice(0,-1);
label=s2.length<t.name.length?s2+'…':t.name;}
ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillText(label,cx+1,cy+1);
